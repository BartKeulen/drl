...
class DQN(object):
    ...
    def __init__(self,
        ...
        self.training_network = DQNNetwork(self.n_actions, n_obs=self.n_obs, network_type=self.network_type,
                                           network_name='Training', options_in=dqn_network_options)
        self.target_network = DQNNetwork(self.n_actions, n_obs=self.n_obs, network_type=self.network_type,
    ...
    def select_action(self, current_state):
        ...
        if random.random() < self.epsilon:
            ...
            index = np.argmax(
                self.training_network.get_Q_Value().eval(feed_dict={self.training_network.input: [current_state]}))
        ...
        for n in range(len(states)):
            ...
            if not terminal_states[n]:
                target = rewards[n] + self.discount_factor * np.amax(
                    self.target_network.get_Q_Value().eval(feed_dict={self.target_network.input: [new_states[n]]}))
            targets.append(target)

        feed_dict = {self.training_network.input: states, self.training_network.target_Q_Value: targets,
                     self.training_network.actions: actions}
        train_value, loss_value, q_value = self._sess.run(
            [self.train, self.training_network.loss, self.training_network.Q], feed_dict=feed_dict)
