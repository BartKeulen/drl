...
if __name__ == '__main__':
	...
	if mode == 'train':
		if not os.path.exists('tmp_MC_reduced_actions_repeat4/Videos'):
			os.makedirs('tmp_MC_reduced_actions_repeat4/Videos')

		populate_replay_buffer(50000)
		while not stopping_condition:
			if episodes % 100 == 0:
				video_recorder = VideoRecorder(env, 'tmp_MC_reduced_actions_repeat4/Videos/MC_' + str(episodes) + '.mp4', enabled=True)
			...
			while not done:
				...
				if done:
					if (episodes % 50 == 0):
						dqn.save('tmp_MC_reduced_actions_repeat4/training.ckpt')
					epsilon = dqn.get_epsilon()
					loss = dqn.get_loss()
					with open('tmp_MC_reduced_actions_repeat4/training_log.csv', 'a', newline='') as csvfile:
		...
		dqn.restore('tmp_MC_reduced_actions_repeat4/training.ckpt')
		...
		episode_reward = 0
		t = 0
		video_recorder = VideoRecorder(env, 'tmp_MC_reduced_actions_repeat4/MC.mp4', enabled=True)
		while not done:
			video_recorder.capture_frame()
